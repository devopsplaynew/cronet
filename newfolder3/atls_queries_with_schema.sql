...
FROM (
    ...
    SELECT 
        ...
        pe.business_dt
    FROM adu.pricing_events pe
    LEFT JOIN adu.pricing_markers pm ON pe.id = pm.pricing_event_id
...
GROUP BY client_cd, processing_region_cd, business_dt
    ...
    SELECT 
        ...
        ae.business_dt
    FROM adu.accounting_events ae
    LEFT JOIN adu.markers m ON ae.id = m.accounting_event_id
    ...
    SELECT 
        ...
        MAX(ae.created_at) AS last_updated,
        ae.business_dt
    FROM adu.as_of_events ae
    LEFT JOIN adu.as_of_request_markers arm ON ae.id = arm.as_of_event_id
