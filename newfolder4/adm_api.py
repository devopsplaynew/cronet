# adm_api.py - Final with AOD workflow fixes (counts restored)
...
EXPECTED_SUBJECTS = {
    ...
    "aod_final": ["positions", "taxlots"],  # special handling
...
def evaluate_aod_stage(rows, total_count):
    """Evaluate AOD raw/enrich/roll/mart with counts."""
...
def evaluate_aod_final(rows, total_count):
    """Evaluate AOD final (positions + taxlots + global marker)."""
    positions_ids = {
...
def get_atls_message_ids(business_date, sod_date):
    """Get message IDs from ATLS for business_date + sod_date."""
    message_ids_map = {}
    try:
    ...
    try:
        ...
        for client, region in clients_regions:
            ...
            for workflow_type, expected_subjects in EXPECTED_SUBJECTS.items():
                ...
                # ✅ AOD Final includes global marker
                if snapshot == "AOD" and stage == "FINAL":
                    workflow_rows = [
                        r for r in all_rows
                        if r["client_cd"] == client
                        and r["processing_region_cd"] == region
                        and r["snapshot_type_cd"].upper() == snapshot
                        and (not message_ids or r["original_message_id"] in message_ids
                    ...
                    workflow_rows = [
                        ...
                        and r["processing_region_cd"] == region
                        and r["snapshot_type_cd"].upper() == snapshot
                        and str(r["business_dt"]) == target_date
                        and (not message_ids or r["original_message_id"] in message_ids)
                    ]

                subjects_found = list(set(r["subject_area_cd"] for r in workflow_rows))
                ...
                if snapshot == "AOD":
                    ...
                    # ✅ Stage-specific failure detection
        ...
        logger.info(f"Generated {len(workflows)} ADM workflows total")
        workflows.sort(key=lambda w: WORKFLOW_ORDER.get(w["workflow_type"], 999))
        logger.info(f"Generated {len(workflows)} ADM workflows total (ordered)")
        return workflows
