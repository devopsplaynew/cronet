...
def details(client, region):
    ...
    try:
        ...
        # Single-call reporting tables (reuses same combined SQL inside helper)
        try:
            from api.adm_api import get_all_reporting_loaders_status
            reporting_status = get_all_reporting_loaders_status(client, region, business_date)
        except Exception as e:
            logger.exception("Error getting reporting loaders status: %s", e)
            reporting_status = {}
        ...
        return render_template(
            ...
            reporting_loaders=reporting_status
        )
