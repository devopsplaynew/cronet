...
- reporting_loaders_markers UNION included (uses base_r alias)
- diagnostic logging added to show how many reporting rows were fetched from combined SQL
- REPORTING_STATIC_MARKERS mapping added
- get_all_reporting_loaders_status helper added (single call, reuses combined SQL results)
No other logic changes to workflows, status evaluation, or filtering.
"""
...
# ----------------------------------
# SQL builder (with reporting block alias fix)
# ----------------------------------
...
# --------------------------------------------------------------------
# Reporting helpers (single-call, reuses combined SQL rows)
# --------------------------------------------------------------------
...
def get_all_reporting_loaders_status(client, region, business_date):
    """
    Return reporting loader statuses for all snapshots in one call.
    Reuses the same combined SQL fetch (filtered by client+region) to avoid extra DB round-trips.
    ...
    # fetch combined rows (filtered for client/region)
